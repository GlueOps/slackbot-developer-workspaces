...
5. **`(Over Allocated)` suffix passes through unchanged to the provisioner** — the provisioner appends ` (Over Allocated)` to instance type names when a node is under-resourced. The slackbot displays this label as-is and sends it back to the provisioner as the `instance_type` value in `/v1/create`. The provisioner strips the suffix before VM creation. Do not strip it on the path to the provisioner. **Sole exception:** the cloud-init env file (`GLUEOPS_CDE_INSTANCE_TYPE` in `/etc/glueops/codespace.env`, built in `util/libvirt/libvirt-server.js`) strips the suffix so the VM records the canonical instance type rather than a transient scheduling condition. This does not affect what `/v1/create` receives.
